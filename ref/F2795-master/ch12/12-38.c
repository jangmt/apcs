...
int delete(char name[]) {
  NODE **prev_next = &head;  // 指向前一個節點的 next
  NODE *temp;                // 指向找到的節點

  while (*prev_next != NULL) {
    if (strcmp((*prev_next)->name, name) == 0) {  // 若字串相同…
      temp = (*prev_next);                        // 記錄找到的節點
      *prev_next = (*prev_next)->next;            // 讓前一節點的 next 指向下個節點
      free(temp);                                 // 釋放（刪除）當前節點

      return 0;  // 代表刪除成功
    }
    prev_next = &((*prev_next)->next);  // 往下一個節點搜尋
  }
  return -1;
}
